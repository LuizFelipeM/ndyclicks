...
const Splitter: React.FC<SplitterProps> = ({ slice }) => {
  ...
  useEffect(() => {
    ...
    containerRef.current?.addEventListener("touchmove", handleDrag, { passive: false })

    return () => {
      containerRef.current?.removeEventListener("mousemove", handleDrag)
      containerRef.current?.removeEventListener("touchmove", handleDrag)
    }
  }, [])

  useEffect(() => setImagesMinWidth(`${containerRef.current?.offsetWidth}px`), [containerRef.current?.offsetWidth])
  ...
  return (
    ...
    >
      ...
      >
        {/* Line */}
        <div className="h-full w-[2px] bg-white-smoke" />

        {/* Resize icon */}
        <div className="
          bg-jet bg-opacity-70
          absolute flex
          justify-center items-center cursor-ew-resize
          -left-4 top-1/2
          transform -translate-y-1/2
          w-8 h-8
          border-2 border-white-smoke rounded-full">
          <div className="w-2 h-2 flex gap-1 justify-center items-center">
      ...
      <div className="absolute top-2 right-2 z-40 text-xs text-white-smoke bg-rich-black rounded-full py-2 px-3">
        Depois
      </div>
      <div className="absolute top-2 left-2 z-40 text-xs text-white-smoke bg-rich-black rounded-full py-2 px-3">
        Antes
      </div>
    </section>
  )
